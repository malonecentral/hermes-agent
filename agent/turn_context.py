...
def build_turn_context(
    ...
    *,
    ...
    # exactly the bytes the loop sends. codex_app_server consumes this same
    # sidecar directly when starting its turn, keeping composition independent
    # of the answer transport. MoA turns append per-call aggregated reference
    # context to the same API copy AFTER this composition, so the stamped bytes
    # wire either — skip the stamp rather than persist provably wrong "exact
    # sent bytes" (MoA keeps its pre-sidecar cache behavior).
    if (
        (not moa_active or getattr(agent, "api_mode", None) == "codex_app_server")
