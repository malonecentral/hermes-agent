...
def normalize_document_chunk(result, container_tag: str, hydrated_parent) -> dict | None:
    ...
    source_container = (
        "family_shared" if visibility == "family_shared"
        else "owner_primary" if visibility == "owner_private"
        else ""
    )
    expected_custom_id = (
    ...
    hydrated_id = field(hydrated_parent, "id")
    custom_id = field(hydrated_parent, "custom_id", field(hydrated_parent, "customId", ""))
    ...
    if document_id and document_id != parent_id:
        return None
    hydrated_tags = field(hydrated_parent, "container_tags",
                          field(hydrated_parent, "containerTags"))
    if hydrated_tags != [container_tag]:
        return None
